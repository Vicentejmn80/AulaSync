...
@section('content')
    ...
    <p class="sub">Fallos del chat, documentos y latencia, colegio por colegio. Los procesos automáticos sin colegio quedan en la franja de plataforma.</p>

    @if (($schoolDossiers ?? collect())->isNotEmpty())
        <p class="section-kicker">Salud por colegio</p>
        <div class="school-stack">
            @foreach ($schoolDossiers as $school)
                @php $health = $school['health']; $isStable = \App\Support\SuperAdminCopy::status($health['estado']) === 'Estable'; @endphp
                <x-super-admin-school-card :school="$school">
                    <div class="grid">
                        <div class="stat">
                            ...
                            <b>{{ \App\Support\SuperAdminCopy::status($health['estado']) }}</b><span>Estado del colegio</span>
                        </div>
                        <div class="stat">
                    ...
                    <div class="card">
                        ...
                        @if ($health['errores_recientes']->isEmpty())
                            <p class="empty">No hay errores en este colegio.</p>
                        @else
                            <table>
                                ...
                                <tbody>
                                    @foreach ($health['errores_recientes'] as $row)
                                        <tr>
                                            ...
                                            <td><span class="status-badge failed">{{ \App\Support\SuperAdminCopy::error($row->error_code) }}</span></td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    <details class="card platform-strip" open>
        <summary style="cursor:pointer;font-weight:800;">Salud de plataforma</summary>
        ...
        <div class="card" style="margin-top:12px;margin-bottom:0;">
            ...
            @else
                <table>
                    <thead><tr><th>Cuándo</th><th>Colegio</th><th>De dónde</th><th>Qué intentó</th><th>Qué pasó</th></tr></thead>
                    <tbody>
                        @foreach ($health['errores_recientes'] as $row)
                            <tr>
                                <td>{{ $row->created_at?->format('d/m H:i') }}</td>
                                <td>{{ $row->colegio_id ? '#'.$row->colegio_id : 'Sin colegio' }}</td>
                                <td>{{ \App\Support\SuperAdminCopy::source($row->source) }}</td>
                                <td>{{ \App\Support\SuperAdminCopy::action($row->action) }}</td>
                                <td><span class="status-badge failed">{{ \App\Support\SuperAdminCopy::error($row->error_code) }}</span></td>
                            </tr>
