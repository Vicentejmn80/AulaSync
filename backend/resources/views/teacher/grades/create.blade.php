...
<head>
    ...
    <style>
        ...
        :root:not(.dark) .text-white { color: var(--text-primary); }
        :root:not(.dark) .text-white\/30 { color: var(--text-tertiary); }
        :root:not(.dark) .text-white\/40 { color: var(--text-tertiary); }
        :root:not(.dark) .text-white\/50 { color: var(--text-tertiary); }
        :root:not(.dark) .text-white\/60 { color: var(--text-secondary); }
        :root:not(.dark) .hover\:bg-white\/10:hover { background: var(--nova-glass); }
        :root:not(.dark) .hover\:bg-white\/20:hover { background: var(--nova-glass); }
        :root:not(.dark) .text-slate-200 { color: var(--text-secondary); }
        :root:not(.dark) .text-violet-400,
        :root:not(.dark) .text-violet-300,
        :root:not(.dark) .text-violet-100,
        :root:not(.dark) .text-cyan-400,
        :root:not(.dark) .text-emerald-400,
        :root:not(.dark) .text-amber-400 { color: var(--nova-violet); }
        :root:not(.dark) .bg-violet-900\/30 { background: rgba(124,58,237,.08); }
        :root:not(.dark) .bg-slate-700,
        :root:not(.dark) .hover\:bg-slate-600:hover { background: var(--bg-tertiary); }
        :root:not(.dark) .grade-input { background: var(--bg-tertiary); color: var(--text-primary); border-color: var(--nova-glass-border); }
        :root:not(.dark) .bg-slate-900 { background: var(--bg-secondary); }
...
function gradeManager() {
    return {
        ...
        students: @json($students->map(fn($s) => [
            'id' => $s['id'], 
            'name' => $s['name'], 
            'score' => $s['existing_score'] ?? null, 
            'nota_actual' => $s['nota_actual'] ?? null
        ])),
